...
def mean_channels(F):
    assert(F.dim() == 4) # [batch_size, channels, height, width]
...
class DirectAwareAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        combined_output = self.apply_fourier_attention(x)
...
class DilateBlock(nn.Module):
    def __init__(self, channels):
        ...
        # padding == dilation so output dimension is maintained
...
class One2One_noaux_attention(nn.Module):
    def __init__(self, in_channels=3, short_connection=True):
        super(One2One_noaux_attention, self).__init__()
